...
from torchmetrics import Accuracy, Precision, Recall, F1, AUROC
...
def train_model(model,training_params):
    ...
    for epoch in range(1, training_params['epochs']+1):
        ...
        for i, batch in enumerate(tqdm(training_params['train_loader'], disable=DISABLE_TQDM), start=1):
            ...
            if i%training_params['evaluate_every'] == 0:
                valid_loss, metrics = eval_one_epoch(model,training_params['valid_loader'])
...
def main:
    """
    Finetunes the Vision Transformer model on the Places365 Dataset
    ...
    train_dataset, valid_dataset = torch.utils.data.random_split(dataset, \
        [int(dataset_len)-len(test_df), len(test_df)], \
    ...
    wandb.init(
        dir= "/scratch2/knarasim/places365test",
        project="Places365Test",
        notes="obtain baseline",
        tags=["baseline"],
        config=wandb_config
    )

    train_loader = DataLoader(train_dataset, batch_size=config['batch_size'], shuffle=True, num_workers=8,collate_fn=collate_fn)
    valid_loader = DataLoader(valid_dataset, batch_size=config['batch_size'], shuffle=True, num_workers=8,collate_fn=collate_fn)
